...
class KeyboardDetector:
    ...
    def detect(self, hand_landmarks, handness_label):
        ...
        if self.__is_detected:
            return True
        pred = self.predict(hand_landmarks)
        if pred == 1:
...
class TouchDetector:
    ...
    def __get_min_dist_key_and_dist(self, touch_finger_pos: list, keyboard_pos_dict: dict):
        ...
        # 距離が最小のキーを取得
        min_dist_key = min(dist_between_touch_finger_and_keyboard_dict,
    ...
    def detect(self, hand_landmarks, handness_label):
        if self.__touch_dist_threshold == 0.0:
            ...
            # タッチする指の座標を取得
            finger_index_landmarks = hand_landmarks.landmark[self.__TOUCH_FINGER_INDEX[0]]
            touch_finger_pos = [
